...
namespace Corebase\Filemanager\src\Controllers;

use App\Http\Controllers\Controller;
use Corebase\Filemanager\src\Classes\SimpleImage;
...
class FilemanagerController extends Controller
{
    ...
    private function postUpdateName(Request $request)
    {
        ...
        try {
            ...
            if (in_array($oldData->ext, $imageExtensions)) {
                ...
                $fromTiny = $this->basePath . '/tinies/' . $oldData->name;
                $toTiny = $this->basePath . '/tinies/' . $finalName;
                $fromlarge = $this->basePath . '/larges/' . $oldData->name;
                $tolarge = $this->basePath . '/larges/' . $finalName;
                File::move($fromThumb, $toThumb);
                File::move($fromTiny, $toTiny);
                File::move($fromlarge, $tolarge);
            }
    ...
    public function makeTiny($sourcePath)
    {
        $filename = basename($sourcePath);
        $pathTinyToSave = $this->basePath . '/tinies';

        if (!file_exists($pathTinyToSave)) {
            mkdir($pathTinyToSave, 0777, true);
        }

        $savePathWithName = $pathTinyToSave . '/' . $filename;
    ...
    public function makeLarge($sourcePath)
    {
        $filename = basename($sourcePath);
        $pathTinyToSave = $this->basePath . '/larges';
    ...
    public function postUpload(Request $request)
    {
        ...
        try {
            ...
            if (in_array($extension, $imageExtensions)) {
                ...
                $this->makeTiny($finalFilePath);
                $this->makeLarge($finalFilePath);
            }
        } catch (\Exception $e) {
            return $this->handleRollback($e, $fileName);
        }
    ...
    public function deleteFile($fileName)
    {
        try {
            ...
            //delete tiny
            File::delete($this->basePath . '/tinies/' . $fileName);
            //delete large
            File::delete($this->basePath . '/larges/' . $fileName);
